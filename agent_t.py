...
class Agent:
    def __init__(self, batch_size):
        ...
        self.actor = Actor(self.dim_state, self.dim_action, ACTION_BOUND , TAU, LEARNING_RATE, batch_size)
        self.critic = Critic(self.dim_state, self.dim_action, self.dim_env, ACTION_BOUND , TAU, LEARNING_RATE)#, self.actor.get_num_trainable_vars())

        # Noise Process
        self.action_noise = OrnsteinUhlenbeckActionNoise(mu=torch.zeros(self.dim_action))

        # Target Networks
        self.target_actor = Actor(self.dim_state,  self.dim_action, ACTION_BOUND, TAU, LEARNING_RATE, batch_size)
        self.target_critic = Critic(self.dim_state,  self.dim_action, self.dim_env, ACTION_BOUND, TAU, LEARNING_RATE)#, self.actor.get_num_trainable_vars())
        self.actor.initialize_target_network(self.target_actor)
        self.critic.initialize_target_network(self.target_critic)

    def evaluate_actor(self, obs , history):
        return self.actor(obs, history)

    def evaluate_critic(self, obs, action, env, history):
        ...
        return self.critic(env, obs,  action, history)

    def train_critic(self, obs, action,  env, history, predicted_q_value):
        self.critic.train(env, obs, action, history, predicted_q_value)

    def train_actor(self, obs, history, a_gradient):
        self.actor.train(obs, history, a_gradient)
    ...
    def update_networks(self, episodes, gamma):
        for episode in episodes:
            ...
            with torch.no_grad():
                target_actions = self.target_actor(states, history)
                envs=torch.tensor(envs,dtype=torch.float32,requires_grad=True)
                target_q_values = self.target_critic(envs, target_actions[0], states,  history)
            ...
            # print(y_i)
            # Train Critic
            self.train_critic(states, actions,  envs, history, y_i)

            # Update Actor
            predicted_actions = self.actor(states,  history)
            predicted_actions_=predicted_actions.clone().detach()
            predicted_actions_[0].requires_grad=True
            action_gradients = self.critic.action_gradients(envs, states,  predicted_actions[0], history)
            self.train_actor(states, history, action_gradients)

        # Soft update target networks
        self.update_target_networks()


    def evaluate_policy(self, experiment, num_rollouts, max_steps):
        success_count = 0
        total_reward = 0
        for _ in range(num_rollouts):
            ...
            while not truncated:
                action = self.evaluate_actor(torch.from_numpy(obs).type(torch.float32),episode.get_history())
